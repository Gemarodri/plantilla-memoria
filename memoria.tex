...
En primer lugar me gustaría empezar agradeciendo a mi familia todo el apoyo constante que he recibido siempre, y la confianza que han depositado en mi desde que decidí adentrarme en el mundo de las Telecomunicaciones. Pero sobre todo me gustaría dejar constancia de lo agradecida que me siento con mis padres, ya que ellos me han sufrido en mis épocas de estrés, y a pesar de eso, siempre me han regalado palabras de ánimo y me brindan grandes consejos constantemente. <Gracias Padres!
...
Llegados a este punto, y mirando todo lo que las telecomunicaciones me ha ofrecido, si volviese a elegir una carrera, no dudo que sería la misma. Las Telecomunicaciones me han ofrecido grandes amigos, experiencias inolvidables como el Erasmus, la importancia del esfuerzo constante, y la posibilidad de conocer a esos grandes profesores, profesores de vocación que hacen que este mundo de la ingeniería y las Telecomunicaciones sea fascinante.

Por último, me gustaría agradecer la ayuda recibida por parte de mi tutor, Jesús, y de mis compañeros.
...
Estamos involucrados en un proyecto de investigación relacionado con el campo de conocimiento del Software, que se centra en área del Bug-Seeding. Como en la mayoría de proyectos de este estilo, es esencial disponer de herramientas que nos faciliten el trabajo de una manera u otra. Por ese motivo, y tras obtener una metodología concreta, surgió la idea de desarrollar una herramienta que nos proporcionase ayuda en nuestro análisis.

En esta memoria se presenta tanto el proyecto de investigación con sus primeras conclusiones como la herramienta que hemos desarrollado actuando de apoyo a nuestra investigación. Las primeras conclusiones y datos obtenidos en este estudio servirán como punto de inflexión para debatir si podemos continuar con la investigación. 

La herramienta usa diversas tecnologías actuales, que en capítulos posteriores explicaremos, pero se basa principalmente en el uso de tres tecnologías, HTML5 y JavaScript del lado del cliente, y Node.js del lado del servidor. Además, el propio estudio de investigación nos servirá como método de validación para nuestra herramienta.     
...
We are involved in a research project, which is related to the area of knowledge of Software that focusing in Bug-Seeding. In this project, we carry out an empirical experiment where analyze the importance of a commit antecesor when a bug was inserted in the code. Generally the tools that facilitate the work in any research project are essentials. For this reason, and after obtained a specific methodology, the idea to develop a tool, that provide us support in our analysis, arose.
...
El software libre y de código abierto, \textit{FLOSS}, nos ofrece la gran ventaja de mejorar continuamente su diseño; los desarrolladores estudian, modifican y mejoran el software gracias a la disponibilidad del código fuente. La evolución de la ingeniería del software libre, ha favorecido la aparición de repositorios de versiones, en los cuales se puede observar la evolución del software, permitiendo de esta manera a los desarrolladores investigar como se lleva a cabo el proceso del desarrollo software. 

En sistemas  \textit{FLOSS} todo el mundo es bienvenido para contribuir en el desarrollo, tanto desarrolladores noveles como expertos, todos ellos contribuirán a mejorar el software. Por tanto es imposible tratar de prevenir las necesidades de todos ellos así como los errores que aparecerán. Cientos de desarrolladores tendrán que estar activos en las actividades de mantenimiento, corrigiendo errores, añadiendo nuevas características o mejorando las existentes. Por este motivo, se encontró la necesidad de desarrollar el proceso de Bug-Fixing, proceso en el cual se identifica el error y quien contribuyó en su arreglo.    

Nuestra contribución se centra en el concepto de bug seeding commit. El concepto Bug Seeding evalúa la cantidad de errores software que residen en un sistema, hallando el momento en el cual el error fue introducido al sistema. [hablar mas sobre bug seeding mencionando herramientas o algo]. El pensamiento actual acerca de la responsabilidad de introducir el error en el código, señala al commit anterior como el responsable, como nos muestran los estudios como los desarrollados por [whitehead, zhimerber] , esta idea preconcebida se encuentra tan generalizada que algunos estudios de investigación parten de ella [mencionar los estudios]. Hasta el momento no hemos descubierto ningún estudio ó investigación empírica que trate de contribuir a este pensamiento generalizado, aportando datos ó porcentajes que muestren en cuantos casos la literatura actual es cierta y en cuantos casos, esta idea no es acorde con los datos.

Es debido a esta carencia en la literatura actual, decidimos desarrollar un estudio. En la investigación se realiza un estudio empírico, explicado con mayor detalle en la sección  ~\ref{sec:estudioexp}, enfocado a descubrir el momento en el que un cambio en el código fuente provocó un error, y a partir de ese momento, poder identificar al/los responsables de dicho error, consiguiendo finalmente una evidencia empírica sobre el número de errores introducidos cuyo responsable fue el commit anterior, y aquellos errores introducidos en los que el commit anterior no tiene ninguna responsabilidad.

Analizar un repositorio de versiones como es \textit{SVN} ó \textit{GIT}, implica la repetición constante de una metodología concreta. Por ello nos vimos en la necesidad de desarrollar una herramienta, descrita en la sección \ref{sec:herramienta}, capaz de automatizar la parte cuya repetitividad era elevada, reduciendo así el tiempo de aná lisis. Dicha herramienta es capaz de obtener la información necesaria para realizar nuestro estudio a partir de cierta información relacionada con la notificación de un error.

En esta memoria presentamos nuestro primer estudio en el tema de investigación, así como el diseño de una herramienta desarrollada con tecnologías actuales como son HTML5 y Node.js, que nos ayudará posteriormente en la parte de investigación. Esta primera investigación nos servirá para poder validar nuestra herramienta y a la vez, para decidir si nuestro estudio va encaminado por buen camino. 
...
La literatura actual acerca de la inserción de errores en el código fuente, asienta sus bases en la idea preconcebida de que el error ha sido causado por el commit inmediatamente anterior que cambió las líneas que se vieron involucradas. A partir de esta asunción, se desarrollan multitud de estudios y experimentos relacionados con el Bug-Seeding, Bug-Fixing, ó cualquier tema que involucre el concepto de encontrar quién introdujo el error en el código. 
...
Mientras que en \cite{izquierdo2011developers} se investiga que porcentaje de desarrolladores arreglan sus propios bug sin indicar a los commits anteriores como responsables del error. En  semanal como responsables a los commits previos.
...
La aplicación no es mas que una web dinámica en la que disponemos de tres pestañas  \textit{'Analyse', 'Statistics'} y  \textit{'Modify'}. Son en estas pestañas en las cuales se encuentran solventadas las necesidades enumeradas anteriormente. 
...
La primera captura de pantalla que nos encontramos, imáagen ~\ref{figura:Index}, muestra el aspecto inicial de nuestra aplicación Web. En ella podemos observar en la parte superior a la derecha, las tres pestañas que hemos comentado anteriormente. Por defecto la pestaña 'Analise' se muestra al abrir la aplicación.
\begin{figure}
    \centering
...
En esta sección detallaremos cada una de las funcionalidades que nos ofrecen los botones así como el contenido que aparece en los formularios disponibles en la pestaña  \textit{'ANALYSE'} de nuestra applicación web.
...
Ahora es el momento de evaluar todos los resultados obtenidos en las dos secciones previas, y decidir cual será el el futuro de nuestro trabajo de investigación.
